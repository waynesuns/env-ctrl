...
$.TextboxList.Autocomplete = function(textboxlist, _options){
  ...
  var index, prefix, method, container, list, values = [], searchValues = [], results = [], placeholder = false, current, currentInput, hidetimer, doAdd, currentSearch, currentRequest;
	...
	var init = function(){
		...
		container = $('<div class="'+ prefix +'" />').width(textboxlist.getContainer().actual( 'width' )).appendTo(textboxlist.getContainer());
	...
	var showResults = function(search){
		var results = method.filter(values, search, options.insensitive, options.maxResults,options.skipValues);
...
$.TextboxList.Autocomplete.Methods = {
	...
	standard: {
		filter: function(values, search, insensitive, max,skipValue){
			...
			for (var i = 0; i < values.length; i++){
				...
				if (regexp.test(values[i][1]) && skipValue[values[i][1]]==null) newvals.push(values[i]);
			}
