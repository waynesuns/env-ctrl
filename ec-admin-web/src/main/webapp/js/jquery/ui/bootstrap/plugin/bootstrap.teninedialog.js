...
(function($){
	$.fn.teninedialog = function(options){
		...
		return this.each(function(){
			var obj=$(this);
			modalID=getModalID();
			var tmpHtml='<div id="{ID}" class="modal  fade" role="dialog" aria-labelledby="myModalLabel"  aria-hidden="true"  data-backdrop="static"><div class="modal-dialog"><div class="modal-content"><div class="modal-header text-center"><h4 id="myModalLabel">{title}</h4></div><div class="modal-body text-center"><p>{body}</p></div><div class="modal-footer">{button}</div></div></div></div>';
            ...
            for(var i=0;i<options.otherButtons.length;i++){
            	buttonHtml+='<button style="margin-left:10px" buttonIndex="'+i+'" class="'+btnClass+' btn '+options.otherButtonStyles[i]+'">'+options.otherButtons[i]+'</button>';
            }
            //替换模板标记
