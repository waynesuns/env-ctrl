...
$.TextboxList = function(element, _options){
	...
	var focusFirst = function(){
		var firstElement = list.children().filter(':first');
		if (firstElement) getBit(firstElement).focus();
		return self;
	};
	var getFirst = function(){
		var firstElement = list.children().filter(':first');
		return getBit(firstElement);
	};
	...
	var resetValues = function(){
		...
		index=[];
		create('editable').inject(list);
	};
	
	var setValues = function(values){
		if (!values) return;
		$.each(values, function(i, v){
			if (v) add.apply(self, $.isArray(v) ? [v[1], v[0], v[2]] : [v,v,v]);
		});		
	};
	...
	this.onRemove = function(bit){
		...
		if (prior && prior.is('editable')) {
			var next = getBit(bit.toElement().next());
			if(next && next.is('editable')){
				var val = next.getValue();
				val[1]=(prior.getValue()[1]+val[1]);
				next.setValue(val);
				prior.remove();
...
$.TextboxListBit = function(type, value, textboxlist, _options){
	...
	var self = this;
	var init = function(){
		prefix = textboxlist.getOptions().prefix + options.bitPrefix;
		typeprefix = prefix + '-' + type;
		bit = $('<li style="height:18px"/>').addClass(prefix).addClass(typeprefix)
		...
		if (type == 'box'){
			...
			if (options.deleteButton){
				...
				close = $('<a class="'+ typeprefix +'-deletebutton" />').click(remove).appendTo(bit);
			}
